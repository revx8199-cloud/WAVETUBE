...
// ── PANEL VIP ────────────────────────────────────────────────────────────
const VIP_BADGE_COLORS=['#ffd700','#ff6b35','#c084fc','#4ade80','#f472b6','#fb7185','#facc15','#a78bfa','#22d3ee','#ef4444','#84cc16','#e879f9','#fb923c','#14b8a6','#eab308','#f43f5e'];
  ...
  body.innerHTML=`
    ...
    <div style="margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid var(--border)">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px">
        ...
        ${VIP_BADGE_COLORS.map(c=>`<div onclick="saveAvatarFrame('${c}')" style="width:34px;height:34px;border-radius:50%;background:${c};cursor:pointer;border:3px solid ${myAvatarFrame===c?'#fff':'transparent'};display:flex;align-items:center;justify-content:center">${myAvatarFrame===c?'<svg viewBox="0 0 24 24" width="14" height="14" fill="#000"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>':''}</div>`).join('')}
      </div>
    </div>
...
async function loadVipEmails(){
  const{data}=await sb.from('profiles').select('email,vip_badge_color').eq('is_vip',true);
...
async function loadMyNameColor(){
  if(!currentUser){myNameColor='';myNameFont='';myDisplayNick='';myAvatarFrame='';return;}
  const{data}=await sb.from('profiles').select('name_color,name_font,name,avatar_frame').eq('id',currentUser.id).single();
...
async function showChannel(userId,nameIn,avatar,email){
  ...
  let chNameColor='',chNameFont='',savedDesc=savedDescFallback,joinedAt='',country='',chAvatarFrame='';
  if(userId){
    const{data:profStyle}=await sb.from('profiles').select('name_color,name_font,description,created_at,country,banner_url,avatar_frame').eq('id',userId).single();
