...
function relativeDate(dateStr){
  ...
  const units=[
    [31536000,isRu?['год','года','лет']:['rok','lata','lat']],
    [2592000,isRu?['месяц','месяца','месяцев']:['miesiąc','miesiące','miesięcy']],
    [604800,isRu?['неделю','недели','недель']:['tydzień','tygodnie','tygodni']],
    [86400,isRu?['день','дня','дней']:['dzień','dni','dni']],
    [3600,isRu?['час','часа','часов']:['godzinę','godziny','godzin']],
    [60,isRu?['минуту','минуты','минут']:['minutę','minuty','minut']]
  ];
  for(const[secs,forms] of units){
    ...
    if(n>=1){
      ...
      return isRu?`${n} ${form} назад`:`${n} ${form} temu`;
    }
  }
  return isRu?'только что':'przed chwilą';
}
  ...
  list.forEach(v=>{
    ...
    <div class="card-bottom">
      ...
      <div class="card-info">
        ...
        <p>${v.likes||0} ${likeIcon()} · ${(v.comments||[]).length} komentarzy · ${relativeDate(v.created_at)}</p>
      </div>
    </div>`;
    el.onclick=()=>openP(v.id);
    g.appendChild(el);
...
function updateSaveButtonUI(videoId){
  ...
  if(menuBtn)menuBtn.textContent=isSaved?'✅ Zapisano':'💾 Zapisz';
  const playerBtn=document.getElementById('player-save-btn');
  if(playerBtn&&cur&&String(cur.id)===idStr){
    playerBtn.innerHTML=isSaved
      ?`<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg> Zapisano`
      :`<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M17 3H7c-1.1 0-2 .9-2 2v16l7-3 7 3V5c0-1.1-.9-2-2-2z"/></svg> Zapisz`;
  }
}
...
function updateWatchLaterButtonUI(videoId){
  ...
  if(menuBtn)menuBtn.textContent=isSaved?'✅ W kolejce':'⏰ Obejrzę później';
  const playerBtn=document.getElementById('player-wl-btn');
  if(playerBtn&&cur&&String(cur.id)===idStr){
    playerBtn.innerHTML=isSaved
      ?`<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg> W kolejce`
      :`<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm.5 5H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg> Obejrzę później`;
  }
}
...
function timeAgo(ts){
  ...
  if(w<4)return `${w} tyg. temu`;
  const mo=Math.floor(d/30);
  if(mo<12)return `${mo} mies. temu`;
  const y=Math.floor(d/365);
  return `${y} ${y===1?'rok':'lata'} temu`;
}
...
async function clearAllNotifications(){
  if(!currentUser)return;
  if(!notificationsList.length){toast('Brak powiadomień do wyczyszczenia');return;}
...
function renderNotifications(){
  ...
  if(!notificationsList.length){list.innerHTML='<div class="notif-empty">Brak powiadomień</div>';return;}
