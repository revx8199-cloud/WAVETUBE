        ...
        <div id="pc-${p.id}" style="display:none;margin-top:12px;border-top:1px solid var(--border-soft);padding-top:12px">
          ${(p.comments||[]).map(c=>`
            <div style="display:flex;gap:8px;margin-bottom:10px">
              ...
              <div><div style="font-size:12px;font-weight:600;display:flex;align-items:center">${esc(c.user)}${verifiedBadge(c.email||'')} <span style="color:var(--text-tertiary);font-weight:400;margin-left:6px">${c.time||''}</span></div><div style="font-size:13px;color:var(--text-secondary)">${esc(c.text)}</div></div>
            </div>`).join('')}
...
async function likePost(postId){
  const likedPosts=new Set(JSON.parse(localStorage.getItem('liked_posts')||'[]'));
  const{data:post}=await sb.from('posts').select('likes').eq('id',postId).single();
  if(!post)return;
  let newLikes=post.likes||0;
  if(likedPosts.has(String(postId))){likedPosts.delete(String(postId));newLikes=Math.max(0,newLikes-1);}
  else{likedPosts.add(String(postId));newLikes++;}
...
async function addPostComment(postId){
  ...
  const{data:post}=await sb.from('posts').select('comments').eq('id',postId).single();
  if(!post)return;
  const comments=[...(post.comments||[]),{user:getMyDisplayName(),text:inp.value.trim(),time:now,ts:Date.now(),col:'#cc0000',avatar:meta?.avatar_url||'',email:currentUser.email||'',user_id:currentUser.id,name_color:myNameColor||'',name_font:myNameFont||'',avatar_frame:myAvatarFrame||''}];
