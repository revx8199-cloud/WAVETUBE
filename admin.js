...
async function checkDiscoState(){
  ...
  if(lastSeenBurstAt===null){
    ...
    if(data.last_burst==='rain')fireRain();
...
function toggleSnowEffect(on){
  if(on&&!snowInterval){
    snowInterval=setInterval(()=>{
      ...
      flake.style.cssText=`position:fixed;top:-20px;left:${Math.random()*100}vw;font-size:${10+Math.random()*18}px;color:var(--text-primary);opacity:${.5+Math.random()*.5};z-index:8500;pointer-events:none;animation:snowFall ${4+Math.random()*4}s linear forwards`;
      document.body.appendChild(flake);
      setTimeout(()=>flake.remove(),9000);
    },200);
  } else if(!on&&snowInterval){
    clearInterval(snowInterval);snowInterval=null;
    document.querySelectorAll('.snow-flake-cleanup').forEach(f=>f.remove());
  }
}
...
async function runConsoleCommand(raw){
  ...
  if(base==='disco'){
    ...
    await fireGlobalBurst('rain');
    consoleLog('🌧️ Deszcz spadł u wszystkich!');
  }
  else if(base==='flash'){
    ...
    consoleLog('  rain               - jednorazowy deszcz 🌧️');
  ...
  else if(base==='ban'||base==='unban'||base==='mute'||base==='unmute'){
    const email=parts[1];
  ...
  if(adminTab==='logs'){
    ...
    body.innerHTML=clearBtnHtml+logs.map(l=>`
      <div class="admin-row" style="align-items:flex-start">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;color:#eee">${esc(l.details)}</div>
