...
const SnowFX=(()=>{
  ...
  let dpr=1,colW=8,cols=0,heightMap=null;
  let particles=[];
  const MAX_PARTICLES=320,MAX_LAYER_H=140,GROW_MINUTES=4;
  ...
  function currentMaxLayer(){
    ...
    return Math.min(MAX_LAYER_H,(elapsedMin/GROW_MINUTES)*MAX_LAYER_H);
  }
  ...
  function drawLayer(){
    ...
    ctx.shadowColor='rgba(200,220,255,.4)';
    ctx.shadowBlur=6;
    ctx.fill();
    ctx.shadowBlur=0;
  }
  ...
  function tick(ts){
    ...
    if(active&&!fadingOut&&ts-lastSpawnTs>22&&particles.length<MAX_PARTICLES){
      lastSpawnTs=ts;
      spawnParticle();
    }
